...
public class Formatter {
    ...
    private ArrayList<String> allVendors;
    int maxRow = 15;
    ...
    public void writeWeek(Sheet sheet, Container[] containers, String header) throws IOException {
        int rowNum = 0;
        int cellNum = 0;
        ...
        for (Container c : containers) {
            ...
            for (ArrayList<Container> address : addresses) {
                if (address.get(0).address() == c.address()) addressRef = address;
            }
            if (addressRef == null) {
        ...
        row.createCell(cellNum++).setCellStyle(styles.get("VENDORHEADER"));
        sheet.addMergedRegion(new CellRangeAddress(rowNum - 1, rowNum - 1, cellNum - 3, cellNum - 1));
        cellNum = 0;
        Double tot = 0.0;
    ...
    private ArrayList<Object> writeAddress(Sheet sheet, ArrayList<Container> containers, String address, int rowNum, int cellNum) {
        ...
        for (int i = 0; i < containers.size(); i++) {
            if (i == containers.size() - 1) {
                ...
                } else if (currVendor != containers.get(i).vendor()) {
                    // this container is the first and is the only of a vendor
                    // write container as vendor total
                    row = sheet.createRow(rowNum++);
                    cell = row.createCell(cellNum++);
            ...
            } else if (containers.get(i).vendor() != containers.get(i + 1).vendor()) {
                if (currVendor != containers.get(i).vendor()) {
                    // this container is the first and is the only of a vendor
                    // write container as vendor total
                    row = sheet.createRow(rowNum++);
                    cell = row.createCell(cellNum++);
